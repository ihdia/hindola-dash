...
def fetchUID(aid):
    ...
    print("# of annotated json file : "+str(len(files)))
    if len(files)>0:
        # data='bhoomi_ANINGYA%20VYAKHYA_PIVS_001_16_Sun Jan 13 16:00:33 2019.json'
        filename=(str(files[0]).rsplit('_',1))[0]

        filepath=pathtojson+str(files[0])
        print(filepath)
        ...
        sql = "INSERT INTO info(username, file,status,imagelinks,dateOfAnnotation) VALUES(%s, %s, %s,%s,%s)"
...
def mydata(data):
    ...
    fullpath=pathtojson+filename+'_'+curr_time+'.json'
    print(fullpath)
    ...
    sql = "INSERT INTO puids(pfile, pfilepath) VALUES(%s, %s)"
...
def reportBack(n):
    ...
    with open(pathtoreportfile,'w') as f:
        f.write(filename)
...
if __name__ == '__main__':
    ...
    socketio.run(app,debug=True,host='10.5.0.142',port=12346)
    # socketio.run(app)
