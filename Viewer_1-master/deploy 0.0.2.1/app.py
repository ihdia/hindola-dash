...
app.config['MYSQL_DATABASE_HOST'] = 'http://10.5.0.142:56000'
mysql.init_app(app)
####################################################
...
def mydata(data):
    ...
    if puidtemp:
        ...
        sql = "UPDATE puids SET pfilepath=%s WHERE pfile=%s"
        d = (newfullpath,filename)
        conn = mysql.connect()
        cursor = conn.cursor()
        cursor.execute(sql, d)
    else:
...
if __name__ == '__main__':
    ...
    # socketio.run(app,debug=True,host='10.5.0.142',port=12348)
    socketio.run(app)
